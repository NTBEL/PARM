...
def pre_equilibration(model, time_search, parameters=None, tolerance=1e-6):
    ...
    solver = ScipyOdeSimulator(model, tspan=time_search, param_values=parameters, integrator='lsoda').run()
    if solver.nsims == 1:
    ...
    for n, y in enumerate(simulations):
        ...
        for idx in range(y.shape[1]):
            ...
            for i in derivative_range_idxs:
                ...
                raise Exception('Species s{0} ({1}) has not reached equilibrium'.format(idx,model.species[idx]))
