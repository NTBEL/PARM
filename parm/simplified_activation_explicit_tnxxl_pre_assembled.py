'''PARM: PAR2 Activation-driven calcium Release Model - pre-assembled variant.
...
This model is a variant of PARM in which the G-protein, G_alphaq (or Gaq), is
initialized pre-assembled on PAR2 at the cell membrane, representing the
presumption that Gaq should be pre-assembled on PAR2 to undergo activation and
subsequent release into the cytosol. This means the Gaq activation rules are
also different; Gaq activation now consists of unidirectional activation by PAR2
followed by unidirectional release from PAR2, and then subsequent unidirectional
translocation of activated-Gaq from the cell membrane to the cytosol.
...
follows:
  ...
  2. Gaq activation by activated-PAR2:  | Note: Gaq is pre-assembled on PAR2.
      i) Activaton of Gaq by activated-PAR2:
         PAR2_A:Gaq_I ---> PAR2_A:Gaq_A
...
KT = 1e-2 # 1/s - the default translocation rate from Albeck et al. https://doi.org/10.1371/journal.pbio.0060299
...
# Initialize with Gaq assembled at the Cell Membrane attached to PAR2
Initial(PAR2(state='I', bgaq=1, btat=None)**CELL_MEMB % Gaq(state='I', b=1)**CELL_MEMB, PAR2_0)
# inactive Gaq
#Parameter('Gaq_0', SPC)
...
# Gaq activation by activated-PAR2, release, and transport to the cytosol
Parameter('kcat_activate_Gaq', KCAT)
Parameter('k_release_Gaq', KR)
Parameter('k_transport_Gaq', KT)
# PLC binding Gaq
...
#Parameter('kr_erCa_bind_IP3R', KR)
Parameter('kcat_tranport_erCa', KT)
#  cytosol -> ER:
...
Parameter('kcat_tranport_cytCa', KT)
...
# PAR2 activation by 2AT:
#    2AT + PAR2_I <---> TAT:PAR2_I ---> TAT + PAR2_A
Rule('tat_bind_PAR2', TAT(b=None)**EXTRACELLULAR + PAR2(bgaq=1, btat=None, state='I')**CELL_MEMB % Gaq(b=1, state='I')**CELL_MEMB |
     TAT(b=2)**EXTRACELLULAR % PAR2(bgaq=1, btat=2, state='I')**CELL_MEMB % Gaq(b=1, state='I')**CELL_MEMB, kf_PAR2_bind_TAT,kr_PAR2_bind_TAT)
Rule('tat_activate_PAR2', TAT(b=2)**EXTRACELLULAR % PAR2(bgaq=1, btat=2, state='I')**CELL_MEMB % Gaq(b=1, state='I')**CELL_MEMB
     >> TAT(b=None)**EXTRACELLULAR + PAR2(bgaq=1, btat=None, state='A')**CELL_MEMB % Gaq(b=1, state='I')**CELL_MEMB, kcat_activate_PAR2)

# Gaq activation by activated-PAR2, release, and transport to the cytosol:
#    PAR2_A:Gaq_I ---> PAR2_A:Gaq_A
#    PAR2_A:Gaq_A---> PAR2_A + Gaq_A
#    Gaq_A_CM ---> Gaq_A_C
Rule('gaq_activation', PAR2(bgaq=1, btat=None, state='A')**CELL_MEMB % Gaq(b=1, state='I')**CELL_MEMB >>
     PAR2(bgaq=1, btat=None, state='A')**CELL_MEMB % Gaq(b=1, state='A')**CELL_MEMB, kcat_activate_Gaq)
Rule('gaq_release', PAR2(bgaq=1, btat=None, state='A')**CELL_MEMB % Gaq(b=1, state='A')**CELL_MEMB >>
     PAR2(bgaq=None, btat=None, state='A')**CELL_MEMB + Gaq(b=None, state='A')**CELL_MEMB, k_release_Gaq)
Rule('gaq_transport', Gaq(b=None, state='A')**CELL_MEMB >> Gaq(b=None, state='A')**CYTOSOL, k_transport_Gaq)
